...
AS
BEGIN
  IF EXISTS (SELECT 1 FROM billing.Bills WHERE patient_insurance_id = @insurance_id)
  BEGIN
...
AS
BEGIN
  IF EXISTS (SELECT 1 FROM billing.Bill_Items WHERE bill_id = @bill_id) OR
...
AS
BEGIN
  IF EXISTS (SELECT 1 FROM billing.Bill_Items WHERE service_id = @service_id)
  BEGIN
...
alter PROCEDURE Doctors_By_Specialization
AS
BEGIN
  SELECT
    s.specialization_id,
